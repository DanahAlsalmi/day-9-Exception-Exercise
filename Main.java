...
//        System.out.println("The character 'a' appears " + count + " times in the sentence.");
    ...
    public static void checkIndex(int index ,String str) throws Exception {
        if(index<0) {
            throw new Exception("Invalid index! Please enter an index between 0 and "+(str.length()-1));
        }

    }
    public static void checkWord(String word) throws Exception {
        if (!word.matches("[a-zA-Z]+")) {
            throw new Exception("Input must be a word containing only letters.");
        }
    }
    public static void checkNegative(double num) throws Exception {
        if(num<0){
